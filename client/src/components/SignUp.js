...
const SignUp = () => {
    ...
    const submitHandler = useCallback((event) => {
        ...
        if(!(/^.{4,12}$/).test(event.target.id.value.trim())){
            alert('ID는 4~12자까지만 가능합니다.');
            return
        }
        if(!(/^.{4,12}$/).test(event.target.name.value.trim())){
    ...
    const duplicateCheck = debounce(useCallback(() => {
        ...
        Axios.post('/api/duplicateCheck', {id : document.getElementById("inputId").value})
        .then(res => {
            if(res.data) {
                setDuplicate(true)
