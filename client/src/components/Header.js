...
const Header = ({ user, pageChange, ua, handleChangeUser, handleChange, handleChangePageNum, handleChangeInput, search, pageSize }) => {
    ...
    return (
        <nav id="boardNav">
    {user.id ? <div style={{display : "inline"}}><button id="leftNav" onClick={logout}>로그아웃</button> <span style={{fontSize : "10px", marginLeft: "10px"}}>{user.userName}</span></div>: <button id="leftNav" onClick={openPopup}>로그인</button>}
                ...
                <Button style={{position : "relative" , top : "-11px"}} id="searchBtn" color="primary" size="sm" type="submit" onClick={debounce(async() => {
                    ...
                    if (ua()) {
                        ...
                        if (boardUl) {
                            ...
                            await pageChange(1, Math.round(window.innerHeight / 86),false)
